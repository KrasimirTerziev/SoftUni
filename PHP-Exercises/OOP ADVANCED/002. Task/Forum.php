...
class Forum
{
   ...
   {
       if(array_key_exists($username, $this->usersByUsename)){
   ...
   public function comment(int $answerId, string $body)
   {
       ...
       if(array_key_exists($answerId,  $this->answers) ){
           throw new Exception("Answer does not exists");
       }
       $answer = $this->answer($answerId);
   ...
   public function show()
   {
       foreach ($this->questions as $question){
           echo " --Question Title" . $question->getTitle() . " Body " . $question->getBody() . " Autho " . $question->getAuthor()->getUsername();
           foreach ($question->getAnswers() as $answer){
               echo"        ---Answer Body: " . $answer->getBody(). "Author" . $answer->getAuthor()->getUsername();
               foreach ($answer->getComments() as $comment){
                   echo"            ---Comment Body: " . $answer->getBody(). "Author" . $answer->getAuthor()->getUsername();
               }
           }
       }
   }
